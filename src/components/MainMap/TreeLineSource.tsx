...
const TreeLineSource: React.FC = () => {
  ...
  const handleMouseMoveTree = (e: MapLayerMouseEvent) => {
    ...
    if (e.features!.length > 0) {
      map.current!.setFeatureState(
        { source: "tree-locations", id: Number(e.features![0].id) },
        { hover: true }
      );
    }
  };
  ...
  const handleMouseLeaveTree = (e: MapLayerMouseEvent) => {
    ...
    treeLocations
      ...
      .features.forEach((f) =>
        map.current!.setFeatureState(
          { source: "tree-locations", id: f.properties.id },
          { hover: false }
        )
      );
  };
    ...
    (e: MapLayerMouseEvent) => {
      ...
      const lineBbox = bbox(
        treeLines
          .peek()
          .features.find(
            (f) => f.properties.id === feature.properties.treeLineId
          )!
      );
  ...
  const handleLineClick = useCallback(
    (e: MapLayerMouseEvent) => {
      ...
      const lineBbox = bbox(
        treeLines
          .peek()
          .features.find((f) => f.properties.id === feature.properties.id)!
      );
  ...
  useEffect(() => {
    const imageUrl = "/markers/bergahorn/bergahorn-10-rm-bg.png";

    if (!map.current) return; // Ensure map.current is defined
