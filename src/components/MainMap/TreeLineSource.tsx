...
const TreeLineSource: React.FC = () => {
  ...
  const handleMouseLeaveTree = (e: MapLayerMouseEvent) => {
    ...
    treeLocations
      .peek()
      .features.forEach((f) =>
        map.current!.setFeatureState({ source: "tree-locations", id: f.properties.id }, { hover: false })
      );
  };
  ...
  const handleMouseLeaveCanopy = (e: MapLayerMouseEvent) => {
    ...
    canopyLayer
      .peek()
      .features.forEach((f) =>
        map.current!.setFeatureState({ source: "canopy", id: f.properties.id }, { hover: false })
      );
  };
      ...
      <Source id="tree-locations" type="geojson" data={treeLocations.value} generateId>
        <Layer
          ...
          layout={{
            ...
            "icon-anchor": "bottom",
            "icon-size": ["interpolate", ["linear"], ["zoom"], 10, 0.1, 12, 0.5, 16, 1, 18, 1.2, 20, 1.4, 22, 1.6],
            //'icon-size': ['case', ['boolean', ['feature-state', 'hover'], false], 1.1, 1],
          }}
        />
        <Layer
          id="canopy-center-layer"
          source="tree-locations"
          type="circle"
